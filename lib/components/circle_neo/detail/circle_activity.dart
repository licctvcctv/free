...
import 'package:freego_flutter/components/circle_neo/detail/circle_activity_apply.dart';
...
class CircleActivityPage extends StatefulWidget{
  final CircleActivity circle;
  const CircleActivityPage(this.circle, {super.key});

  @override
  State<StatefulWidget> createState() {
    return CircleActivityPageState();
  }

}

class CircleActivityPageState extends State<CircleActivityPage>{
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: CircleActivityWidget(widget.circle),
    );
  }
...
class CircleActivityState extends State<CircleActivityWidget> with SingleTickerProviderStateMixin{
        ...
        child: Stack(
          children: [
            Column(
              ...
              children: [
                CommonHeader(
                  center: const Text('寻找驴友', style: TextStyle(color: Colors.white, fontSize: 16),),
                  right: InkWell(
                    onTap: (){
                      menuController?.hideMenu();
                      menuController = null;
  ...
  Widget getMapWidget(){
    ...
    if(trip.points == null || trip.points!.isEmpty){
      return const SizedBox();
    }
    TripPoint point = trip.points!.first;
          ...
          child: Stack(
            children: [
              ClipRRect(
                ...
                child: CircleActivityMapShow(latitude: point.latitude!, longitude: point.longitude!,),
              ),
              Container(
                decoration: const BoxDecoration(
                  ...
                  gradient: LinearGradient(
                    ...
                    colors: [Color.fromRGBO(255, 255, 255, 0), Color.fromRGBO(180, 180, 180, 0.8)]
                  )
                ),
                padding: const EdgeInsets.only(bottom: 10),
                alignment: Alignment.bottomCenter,
                child: const Text('路径地图', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16),),
              )
            ],
          ),
        ),
      ...
      child: Column(
        ...
        children: [
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('活动名称：', style: TextStyle(color: ThemeUtil.foregroundColor, fontWeight: FontWeight.bold, fontSize: 16),),
              Expanded(
                child: Text(circle.name ?? '', style: const TextStyle(color: ThemeUtil.foregroundColor, fontWeight: FontWeight.bold, fontSize: 16),),
              )
            ],
          ),
          Padding(
            ...
            child: Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('报名人数：', style: TextStyle(color: ThemeUtil.foregroundColor, fontWeight: FontWeight.bold, fontSize: 16),),
                Expanded(
                  child: Text('${circle.peopleNum ?? 0}', style: const TextStyle(color: ThemeUtil.foregroundColor, fontWeight: FontWeight.bold, fontSize: 16),),
                )
              ],
            ),
          ),
          circle.expectNumMin == null && circle.expectNumMax == null ?
          const SizedBox() :
          Padding(
            padding: const EdgeInsets.only(top: 10),
            child: Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('人数限制：', style: TextStyle(color: ThemeUtil.foregroundColor, fontWeight: FontWeight.bold, fontSize: 16),),
                Wrap(
                  children: [
                    Text('${circle.expectNumMin ?? ''}', style: const TextStyle(color: ThemeUtil.foregroundColor, fontWeight: FontWeight.bold, fontSize: 16),),
                    const Text(' ~ ', style: TextStyle(color: ThemeUtil.foregroundColor, fontWeight: FontWeight.bold, fontSize: 16),),
                    Text('${circle.expectNumMax ?? ''}', style: const TextStyle(color: ThemeUtil.foregroundColor, fontWeight: FontWeight.bold, fontSize: 16),)
                  ],
                )
              ],
            ),
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                const Text('出发时间：', style: TextStyle(color: ThemeUtil.foregroundColor, fontWeight: FontWeight.bold, fontSize: 16),),
                Expanded(
                  child: Text(DateFormat('yyyy-MM-dd').format(circle.startTime!), style: const TextStyle(color: ThemeUtil.foregroundColor, fontSize: 16, fontWeight: FontWeight.bold)),
                )
              ],
            ),
          ),
          circle.assemblingSpot == null ?
          const SizedBox() :
          Padding(
            padding: const EdgeInsets.only(top: 10),
            child: Row(
              ...
              children: [
                const SizedBox(height: 10,),
                const Text('集合地点：', style: TextStyle(color: ThemeUtil.foregroundColor, fontWeight: FontWeight.bold, fontSize: 16),),
                Expanded(
                  child: InkWell(
                    onTap: (){
                      if(circle.assemblingLat != null && circle.assemblingLng != null){
                        Navigator.of(context).push(MaterialPageRoute(builder: (context){
                          return CommonMapShowPage(address: circle.assemblingSpot ?? '', latitude: circle.assemblingLat!, longitude: circle.assemblingLng!);
                        }));
                      }
                    },
                    child: Text(circle.assemblingSpot!, style: const TextStyle(color: ThemeUtil.foregroundColor, fontWeight: FontWeight.bold, fontSize: 16),),
                  ),
                )
              ],
            ),
          ...
          Padding(
            ...
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('活动详情：', style: TextStyle(color: ThemeUtil.foregroundColor, fontWeight: FontWeight.bold, fontSize: 16),),
                const SizedBox(height: 10,),
                Text(circle.description!, style: const TextStyle(color: ThemeUtil.foregroundColor, fontWeight: FontWeight.bold, fontSize: 16),),
              ],
            ),
          ),
        ],
  ...
  Widget getAuthorInfoWidget(){
    ...
    bool canJoin = trip != null && trip.endDate != null && trip.endDate!.isAfter(DateTime.now());
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          canJoin && (localUser == null || localUser.id != circle.userId) ?
          TextButton(
            style: TextButton.styleFrom(
              padding: EdgeInsets.zero,
              minimumSize: Size.zero,
            ...
            onPressed: () {
              DialogUtil.loginRedirectConfirm(context, callback: (isLogined){
                if(isLogined){
                  Navigator.of(context).push(MaterialPageRoute(builder: (context){
                    return CircleActivityApplyPage(circle);
                  }));
                }
              });
            },
            child: Container(
              ...
              child: const Text('报 名', style: TextStyle(color: ThemeUtil.buttonColor),),
            )
          ) : const SizedBox()
        ],
      ),
...
class CircleActivityMapShow extends StatefulWidget{
  final double latitude;
  final double longitude;
  const CircleActivityMapShow({required this.latitude, required this.longitude, super.key});

  @override
  State<StatefulWidget> createState() {
    return CircleActivityMapState();
  }

}

class CircleActivityMapState extends State<CircleActivityMapShow>{
