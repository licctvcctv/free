...
class CircleArticlePage extends StatelessWidget{
  final CircleArticle circle;
  const CircleArticlePage(this.circle, {super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: CircleArticleWidget(circle),
    );
  }
...
class CircleArticleState extends State<CircleArticleWidget> with SingleTickerProviderStateMixin{
        ...
        child: Stack(
          children: [
            Column(
              ...
              children: [
                CommonHeader(
                  center: const Text('图文消息', style: TextStyle(color: Colors.white, fontSize: 18),),
                  right: InkWell(
                    onTap: (){
                      menuController?.hideMenu();
                      menuController = null;
