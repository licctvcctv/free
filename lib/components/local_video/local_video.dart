...
class LocalVideoPage extends StatefulWidget{
  ...
  State<StatefulWidget> createState() {
    return LocalVideoPageState();
  }

}

class LocalVideoPageState extends State<LocalVideoPage>{
  @override
...
class LocalVideoState extends State<LocalVideoWidget>{
  ...
  void initState(){
    ...
    Future.delayed(Duration.zero, () async{
      bool isGranted = await PermissionUtil().requestPermission(context: context, permission: Permission.storage, info: '希望获取存储权限用于查看手机视频');
      if(!isGranted){
        return;
      }
        ...
        children: [
          ...
          Row(
            children: [
              ...
              InkWell(
                onTap: onTapUpload,
                child: const Icon(Icons.file_upload_outlined, color: ThemeUtil.foregroundColor, size: ACTION_SIZE,),
              ),
              const SizedBox(width: 6,),
              InkWell(
                onTap: onTapInfo,
          ...
          Expanded(
            child: Text(DateTimeUtil.getFileTime(entity.statSync().modified), textAlign: TextAlign.end, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.grey, fontSize: 14),),
          )
        ],
      ),
    );   
  ...
  void onTapUpload(){
    DialogUtil.loginRedirectConfirm(context, callback: (isLogined){
      if(isLogined){
        if(mounted && context.mounted){
          Navigator.of(context).push(MaterialPageRoute(builder: (context){
            return VideoUploadPage(entity: widget.entity, cover: coverPath,);
          }));
        }
      }
    });
